...
AddEventHandler('esx_onlineshop:vender', function(nameventa, labelventa, cantidadventa, valorventa)
	...
	for i=1, #items, 1 do
		if (items[i].name == nameventa and items[i].count >= tonumber(cantidadventa) and tonumber(valorventa)>0 and tonumber(cantidadventa)>0) then
			MySQL.Async.execute('INSERT INTO mercado_ventas (vendedor, objeto, label, cantidad, precio, vendido) VALUES (@b, @nameventa, @labelventa, @cantidadventa, @valorventa, @cantidad)',
			  ...
			  function(affectedRows)
				TriggerClientEvent("esx:showNotification", b, "Has puesto en venta " .. cantidadventa .. " " .. labelventa.. " a " .. valorventa .. "$ por unidad con exito")
...
AddEventHandler('esx_onlineshop:cobrar', function()
	...
	local n=0;
	MySQL.Async.fetchAll('SELECT * FROM mercado_ventas WHERE vendedor = @vendedor AND vendido > 0',
		  ...
		  function(result)
			for i,v in ipairs(result) do 
				...
				if (result[i].vendido > result[i].cobrados) then
					n=n+1
					MySQL.Async.execute('UPDATE mercado_ventas SET cobrados = @vendido WHERE id = @idventa',
					  { 
					  ['idventa'] = result[i].id,
					  ...
					  TriggerClientEvent("esx:showNotification", b, "Cobrado "..result[i].precio*(result[i].vendido-result[i].cobrados).."$ con exito")
		...
		if (n == 0) then
			TriggerClientEvent("esx:showNotification", b, "No hay nada por cobrar")
		end
	end)
	
	
	MySQL.Async.execute('DELETE FROM mercado_ventas WHERE cantidad = cobrados',
		{ },
		function(affectedRows)
...
AddEventHandler('esx_onlineshop:comprar', function(idcompra, cantidadcompra)
	...
	MySQL.Async.fetchAll('SELECT * FROM mercado_ventas WHERE id = @idcompra',
		  { ['idcompra'] = idcompra },
		  function(result)
			...
			if (cantidadcompra <= restantes and cantidadcompra > 0) then
				...
				MySQL.Async.execute('UPDATE mercado_ventas SET vendido = @vendido WHERE id = @x',
					  { 
					  ['x'] = idcompra,
					...
					xPlayer.addInventoryItem(result[1].objeto, cantidadcompra)
					  TriggerClientEvent("esx:showNotification", b, "Comprado con exito")
				...
				TriggerClientEvent("esx:showNotification", b, "No tienes dinero suficiente")
				end
			else
			TriggerClientEvent("esx:showNotification", b, "Cantidad invalida")
			end
...
ESX.RegisterServerCallback('esx_onlineshop:obtenerlistaventas', function(source, cb)
	...
	MySQL.Async.fetchAll('SELECT * FROM mercado_ventas ORDER BY precio ASC', { ['@id'] = playerId }, function(result)
